...
protocol UserLoaderDelegate:class  {
    //func successUtf8(data: String)
    func success(parser: UserParser,response: String)
    func error(error: Error?,response: String)
}

class UserLoader: MyApi  {
    
    var TAG: String = "Api.swift"
    
    var delegate: UserLoaderDelegate? = nil
    
    func setDelegate(context: UserLoaderDelegate) {
        delegate = context
    }
    ...
    func run (url: String, parameters: Parameters,compilation: @escaping (UserParser?) -> ()
        ) {
    ...
    var result: UserParser? = nil
    
    func parser(json: String) -> UserParser {
        
        let mParser = UserParser(content: json)
